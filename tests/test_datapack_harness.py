...
class ProfileTests(unittest.TestCase):
    def test_all_profiles_validate_and_form_one_chain(self) -> None:
        ...
        self.assertEqual(58, len(order))
        self.assertEqual("1.13", order[0])
        self.assertEqual("26.3-snapshot-8", order[-1])
    ...
    def test_snapshot_chain_and_channel_are_explicit(self) -> None:
        profiles = HARNESS.load_profiles()
        chain = HARNESS.resolve_chain("26.3-snapshot-8", profiles)
        self.assertEqual(58, len(chain))
        self.assertEqual("26.2", chain[-9]["version"])
        self.assertEqual("26.3-snapshot-8", chain[-1]["version"])
        ...
        self.assertEqual("116.0", payload["profile"]["data_pack_format"])
